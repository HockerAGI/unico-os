...
/**
 * Roles base (multitenant):
 * owner/admin: todo
 * ops: orders + products
 * sales: orders
 * marketing: marketing + settings (contenido)
  ...
  useEffect(() => {
    async function load() {
      ...
      (orders || []).forEach((o) => {
        ...
        stripeFees += Number(o.stripe_fee || 0); // si no existe en tu schema, queda 0
      });
            ...
            {products.map((p) => (
              <tr key={p.id} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4 font-bold text-slate-800">
                  {p.name}
                  <br />
                  <span className="text-xs font-normal text-slate-400">{p.category || "—"}</span><td className="px-6 py-4 font-mono text-slate-700">{moneyMXN(p.price)}</td>
...
function UsersView({ orgId, setHelp, role }) {
  ...
  useEffect(() => {
    // Nota: para mostrar emails necesitas join con profiles o un view. Aquí mostramos user_id y role (seguro).
    supabase
      .from("org_memberships")
  ...
  const invite = async () => {
    ...
    try {
      /**
       * ✅ Aquí NO podemos crear usuarios admin-side con anon key.
       * Lo correcto:
       *  - Crear una Edge Function / Netlify Function con SERVICE_ROLE
       *  - Usar supabase.auth.admin.createUser() + insertar membership
       *
       * Como ahorita solo estás pidiendo “parte faltante”, dejo el UI listo y un TODO claro.
       */
      alert(
        "Invitación pendiente: necesitas una función server-side con SERVICE_ROLE para crear usuario e insertar membership.\n\nSiguiente paso: te creo ese endpoint cuando me pases tu estructura de /app y /api."
      );
    } finally {
      setBusy(false);
    }
